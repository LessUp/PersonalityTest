import type { Assessment, Answer, DetailedResult, DimensionScore } from './types';
...
export function generateDetailedResult(
  ...
  // 心理健康量表分析
    ...
    result.actionableAdvice = generateMentalHealthAdvice(assessment.id, dimensionScores);
  }
...
// 心理健康建议
function generateMentalHealthAdvice(assessmentId: string, scores: DimensionScore[]): string[] {
  const advice: string[] = [];
  const score = scores[0];
  
  if (!score) return advice;

  if (assessmentId === 'phq9') {
    if (score.level === 'low') {
      advice.push('您的抑郁症状较轻，请继续保持健康的生活方式');
    } else if (score.level === 'medium') {
      advice.push('建议关注自己的情绪变化，适当增加社交和运动');
      advice.push('如症状持续，建议咨询专业人士');
    } else {
      advice.push('您的抑郁症状较为明显，强烈建议寻求专业心理健康支持');
      advice.push('请联系心理咨询师或医生进行进一步评估');
    }
  } else if (assessmentId === 'gad7') {
    if (score.level === 'low') {
      advice.push('您的焦虑水平较低，请继续保持');
    } else if (score.level === 'medium') {
      advice.push('尝试放松技巧如深呼吸、冥想');
      advice.push('规律作息和适度运动有助于缓解焦虑');
    } else {
      advice.push('您的焦虑水平较高，建议寻求专业帮助');
      advice.push('可以尝试认知行为疗法等方法');
    }
  }
